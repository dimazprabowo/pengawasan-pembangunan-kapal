<div>
    ...
    <form wire:submit="save" x-data="{ watchJenisKapal: false }" x-init="$watch('$wire.jenis_kapal_id', (val) => { if(val) setTimeout(() => window.initFlatpickrEdit && window.initFlatpickrEdit(), 150) })">
        ...
        @if($jenis_kapal_id)
        <div class="mt-6">
            <x-laporan-mingguan.kurva-s-form
                :mingguOptions="$mingguOptions"
                :hasKurvaS="$hasKurvaS"
        ...
        <div class="mt-6 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div class="px-5 py-3.5 border-b border-gray-100 dark:border-gray-700 flex items-center gap-2">
                ...
                <span class="ml-auto text-xs text-gray-400 dark:text-gray-500 font-normal">Referensi rencana vs aktual</span>
            </div>
            <div class="p-5">
                <x-kurva-s-chart
        ...
        @else
        <script>console.log('Edit: kurvaSChartData is empty', {{ json_encode($kurvaSChartData) }}, 'hasKurvaS:', {{ $hasKurvaS ? 'true' : 'false' }}, 'jenis_kapal_id:', {{ $jenis_kapal_id ?: 'null' }});</script>
        @endif
        @endif
