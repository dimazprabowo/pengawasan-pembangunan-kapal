...
<div class="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
    <div class="flex items-center justify-between mb-3">
        <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-300">E. Uraian Aktivitas</h4>
        @if(!$readonly)
            <button type="button"
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-900">
                <tr>
                    ...
                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider w-40">PIC</th>
                    @if(!$readonly)
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                @forelse($rows as $rIndex => $row)
                    <tr wire:key="{{ $wireKeyPrefix }}-{{ $rIndex }}">
                        ...
                        @if($readonly)
                            <td class="px-3 py-2 text-sm text-gray-900 dark:text-white">{{ $row['aktivitas'] ?? '-' }}</td>
                            <td class="px-3 py-2 text-sm text-gray-500 dark:text-gray-400">{{ $row['pic'] ?? '-' }}</td>
                        @else
                            <td class="px-3 py-2">
                                <textarea wire:model="{{ $rowsModel }}.{{ $rIndex }}.aktivitas" rows="2"
                            ...
                            <td class="px-3 py-2">
                                <input type="text" wire:model="{{ $rowsModel }}.{{ $rIndex }}.pic"
                                    class="w-full px-2 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded focus:ring-1 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"
                                    placeholder="PIC">
                            </td>
                ...
                @empty
                    <tr>
                        <td colspan="{{ $readonly ? 3 : 4 }}" class="px-3 py-6 text-center text-sm text-gray-400 dark:text-gray-500">Tidak ada data aktivitas</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
