...
<div class="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
    <div class="flex items-center justify-between mb-3">
        <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-300">B. Personel</h4>
        @if(!$readonly)
            <button type="button"
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                @forelse($rows as $rIndex => $row)
                    <tr wire:key="{{ $wireKeyPrefix }}-{{ $rIndex }}">
                        ...
                        @if($readonly)
                            <td class="px-3 py-2 text-sm text-gray-900 dark:text-white">{{ $row['jabatan'] ?? '-' }}</td>
                            <td class="px-3 py-2 text-sm text-gray-900 dark:text-white">{{ $row['status'] ?? '-' }}</td>
                            <td class="px-3 py-2 text-sm text-gray-500 dark:text-gray-400">{{ $row['keterangan'] ?? '-' }}</td>
                        @else
                            <td class="px-3 py-2">
                                <input type="text" wire:model="{{ $rowsModel }}.{{ $rIndex }}.jabatan"
                            ...
                            <td class="px-3 py-2">
                                <input type="text" wire:model="{{ $rowsModel }}.{{ $rIndex }}.status"
                                    class="w-full px-2 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded focus:ring-1 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"
                                    placeholder="Status">
                            </td>
                            <td class="px-3 py-2">
                ...
                @empty
                    <tr>
                        <td colspan="{{ $readonly ? 4 : 5 }}" class="px-3 py-6 text-center text-sm text-gray-400 dark:text-gray-500">Tidak ada data personel</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
