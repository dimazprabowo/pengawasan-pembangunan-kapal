...
>
    ...
    {{-- Kurva S Chart + Detail Table --}}
    @if(!empty($kurvaSChartData) && ($kurvaSChartData['has_rencana'] ?? false))
    <div class="mt-6">
        <x-kurva-s-chart :chartData="$kurvaSChartData" :title="'Kurva S — ' . ($jenisKapalNama ?? '')"/>
    </div>
    ...
    <div class="mt-6 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div class="px-5 py-3 bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 flex items-center gap-2">
            <svg class="w-4 h-4 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>
            </svg>
            <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-300">
                Progress per Work Group — Minggu ke-{{ $kurvaSDetail['minggu_ke'] }}
            </h3>
        </div>
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 dark:bg-gray-900/50">
                        <th class="px-4 py-2.5 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Work Group</th>
                        <th class="px-4 py-2.5 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Bobot (%)</th>
                        <th class="px-4 py-2.5 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Rencana Group (%)</th>
                        <th class="px-4 py-2.5 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Rencana Proyek (%)</th>
                        <th class="px-4 py-2.5 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Realisasi Group (%)</th>
                        <th class="px-4 py-2.5 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Realisasi Proyek (%)</th>
                        <th class="px-4 py-2.5 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Deviasi (%)</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    @foreach($kurvaSDetail['rows'] as $row)
                    @php $devColor = $row['dev_project'] >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'; @endphp
                    <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-700/20">
                        <td class="px-4 py-2 font-medium text-gray-800 dark:text-gray-200">{{ $row['nama'] }}</td>
                        <td class="px-4 py-2 text-center text-gray-600 dark:text-gray-400">{{ number_format($row['bobot'], 2) }}</td>
                        <td class="px-4 py-2 text-right text-gray-600 dark:text-gray-300">{{ number_format($row['group_plan'], 2) }}</td>
                        <td class="px-4 py-2 text-right text-gray-600 dark:text-gray-300">{{ number_format($row['project_plan'], 2) }}</td>
                        <td class="px-4 py-2 text-right text-gray-600 dark:text-gray-300">{{ number_format($row['group_real'], 2) }}</td>
                        <td class="px-4 py-2 text-right text-gray-600 dark:text-gray-300">{{ number_format($row['project_real'], 2) }}</td>
                        <td class="px-4 py-2 text-right font-semibold {{ $devColor }}">
                            {{ $row['dev_project'] >= 0 ? '+' : '' }}{{ number_format($row['dev_project'], 2) }}
                        </td>
                    </tr>
                    @endforeach
                ...
                <tfoot>
                    @php $t = $kurvaSDetail['totals']; $tDev = $t['dev_project']; @endphp
                    <tr class="bg-gray-50 dark:bg-gray-900/50 font-semibold border-t-2 border-gray-200 dark:border-gray-600">
                        <td class="px-4 py-2.5 text-xs font-bold text-gray-700 dark:text-gray-300 uppercase">Total</td>
                        <td class="px-4 py-2.5 text-center text-gray-700 dark:text-gray-300">{{ number_format($t['bobot'], 2) }}</td>
                        <td class="px-4 py-2.5"></td>
                        <td class="px-4 py-2.5 text-right text-gray-700 dark:text-gray-300">{{ number_format($t['project_plan'], 2) }}</td>
                        <td class="px-4 py-2.5"></td>
                        <td class="px-4 py-2.5 text-right text-gray-700 dark:text-gray-300">{{ number_format($t['project_real'], 2) }}</td>
                        <td class="px-4 py-2.5 text-right font-bold {{ $tDev >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
                            {{ $tDev >= 0 ? '+' : '' }}{{ number_format($tDev, 2) }}
                        </td>
                    </tr>
                </tfoot>
            </table>
