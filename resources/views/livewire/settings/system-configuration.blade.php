<div>
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="sm:w-64">
            <x-searchable-select
                ...
                :options="collect($categories)->map(fn($label, $key) => ['value' => $key, 'label' => $label])->toArray()"
        ...
        <div class="flex items-center gap-2 w-full sm:w-auto">
            ...
            @can('configuration_create')
                <x-loading-button wire:click="create" target="create" variant="primary" size="md" loadingText="Memuat..." class="flex-1 sm:flex-none">
                    <x-slot:icon>
                                    ...
                                    @can('configuration_delete')
                                        @if($config->is_editable)
                                            <button wire:click="confirmDelete({{ $config->id }})"
                                                wire:loading.attr="disabled"
                                                wire:target="confirmDelete({{ $config->id }})"
                                                class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 disabled:opacity-50"
                                                title="Hapus">
                                                <svg wire:loading.class="hidden" wire:target="confirmDelete({{ $config->id }})" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                                </svg>
                                                <svg wire:loading wire:target="confirmDelete({{ $config->id }})" class="animate-spin w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                                </svg>
                                            </button>
                    ...
                    <form wire:submit="save">
                        <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">
                                {{ $editMode ? 'Edit Konfigurasi' : 'Tambah Konfigurasi' }}
                            </h3>
    ...
    <x-delete-modal 
        ...
        message="Apakah Anda yakin ingin menghapus konfigurasi"
        :itemName="$deletingConfigKey"
